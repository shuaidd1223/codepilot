...
- `chat`、Web UI 会话和飞书自由文本统一交给 OpenCode + CodePilot MCP 处理；旧的硬编码自然语言分发、澄清 REPL 和前缀确认流不再作为主交互入口。
...
- `OPENCODE_TUI_CONFIG` -> `~/.codepilot/opencode/<项目标识>/tui.json`：TUI theme、scroll、diff、默认关闭 mouse reporting 和 CodePilot 品牌 TUI plugin。
...
OpenCode 套壳品牌、TUI、中文交互规则、默认 agent 和内置 commands 是 CodePilot 工具级定制，随包代码发布。业务项目不需要、也不应该为了使用这个工具在 `AGENTS.toml` 中配置 `[opencode.*]`。业务项目目录不会生成 `.codepilot/opencode/`；运行时文件只写入用户级 `~/.codepilot/opencode/<项目标识>/`，用来落地当前项目的注册名和 MCP 启动配置。
