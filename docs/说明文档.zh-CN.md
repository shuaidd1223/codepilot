...
事件 agent 支持的事件类型取决于事件生产方，模板可读取事件 `payload` 中的字段，例如 `{{ task.id }}`、`{{ task.error }}`。飞书相关事件需要先完成 `[feishu_bot]` 配置并启动 `codepilot feishu start`；MCP 工具注入依赖对应 agent CLI 和 MCP launcher 能力，不应把未配置的 MCP / 飞书能力写成必然可用。
