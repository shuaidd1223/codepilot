...
本文定义 CodePilot 第一阶段工作流命令共享的项目本地状态层。该层服务于后续 `plan`、`explore`、`wiki` 等模式，让它们可以复用上下文、恢复状态，并输出机器可读 artifact。
...
| `.codepilot/context/` | 会话上下文快照，例如输入摘要证据索引。 | 默认保留；后续可按 session 或时间归档。 |
...
- `.codepilot/state/<mode>-state.json`：某个模式最近一次状态，例如 `plan-state.json`。
...
| 字段 | 类型 | 说明 |
| :--- | :--- | :--- |
| `mode` | string | 工作流模式，例如 `plan`、`explore`、`wiki`。 |
| `active` | boolean | 是否为仍在进行的工作流。`true` 时同步写入 `active-workflow.json`。 |
| `current_phase` | string | 当前阶段，例如 `started`、`collecting`、`completed`。 |
...
从第一阶段（v0.7.x）开始，CodePilot 引入了 Agent Kernel 会话状态层，用于追踪完整的 Agent 工作流生命周期。一个会话跨越 `intake → explore → plan → approve → execute → review → recover → deliver` 九个阶段。
...
codepilot workflow status -p <项目名> --mode plan --json
```
...
- SQLite DB 继续负责项目注册任务队列任务执行状态日志索引和会话消息。
- `.codepilot/state/` 只保存跨工作流模式的轻量运行态和 artifact 路径。
- `plan` 等后续命令可先写 project-local artifact，再决定是否创建任务。
