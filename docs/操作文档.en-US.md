...
`--write-workflow` stores the read-only inspection result as `.codepilot/context/inspect-*.json` and lets `workflow next --list --json` expose safe actions such as `create_inspect_tasks`, `promote_inspect_report_<candidate_id>`, and `plan_from_inspect`. Plain `--dry-run` keeps the old no-write behavior.
